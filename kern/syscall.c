...
// En toería un proceso puede disminuir su prioridad sin permisos especiales
// pero no puede incrementarla
static int
sys_disminuir_prioridad(envid_t envid)
{
	struct Env *env;
	int r;
	if ((r = envid2env(envid, &env, 1)))
		...
		return r;
	if (env->prioridad < PRIORMIN) {
		env->prioridad++;
		//		env->prioridades[env->env_runs] = env->prioridad;
		// prioridad más alta es cero y PRIORMIN > 0 es la menor prioridad
	}
	return 0;
}
...
static int
sys_aumentar_prioridad(envid_t envid)
{
	struct Env *env;
	int r;
	if ((r = envid2env(envid, &env, 1)))  // 0 on success, -E_BAD_ENV on error.
		return r;
	env->prioridad =
	        0;  // prioridad más alta es cero y PRIORMIN>0 es la menor prioridad
	return 0;
}

static int
sys_get_prioridad(void)
{
	return curenv->prioridad;
}
...
sys_exofork(void)
{
	...
	newenv->prioridad = curenv->prioridad;
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	case SYS_aumentar_prioridad:
		return sys_aumentar_prioridad(a1);
	case SYS_disminuir_prioridad:
		return sys_disminuir_prioridad(a1);
	case SYS_get_prioridad:
		return sys_get_prioridad();
	case SYS_yield:
		sys_yield();  // No return
	default:
		return -E_INVAL;
